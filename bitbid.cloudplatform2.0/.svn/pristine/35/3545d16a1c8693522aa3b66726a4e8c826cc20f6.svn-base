                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <div class="optional" v-if="scope.row.type === 2 || scope.row.type === 3 || scope.row.type === 4">
                        <span v-if="scope.row.original0ptional" @click="editDialogOptional(scope.$index)">{{scope.row.original0ptional}}</span>
                        <span v-if="!scope.row.original0ptional" @click="editDialogOptional(scope.$index)">可选项</span>
                      </div>
                    </template>
                  </el-table-column>
                  <el-table-column
...
export default {
  ...
  methods: {
    /** 改变模板项类型 */
    changeType (index) {
      this.temTableData[index].value = this.temTableData[index].type === 4 ? [] : ''
      this.temTableData[index].original0ptional = ''
      // this.temTableData[index].optional = [{label: '-10000'}]
      this.temTableData[index].optional = []
    },
    /** 打开编辑可选项弹框 */
    editDialogOptional (index) {
      this.$nextTick(() => {
        if (this.temTableData[index].original0ptional) {
          this.desc = this.temTableData[index].original0ptional
        } else {
          this.desc = ''
        }
      })
    ...
    handleOptionalEvent (operation) {
      if (Object.is(operation, 'save')) {
        if (this.desc !== '') {
          ...
          this.temTableData[this.clickRowIndex].optional = []
          for (let i = 0; i < arr.length; i++) {
            if (arr[i]) {
              this.temTableData[this.clickRowIndex].optional.push({'label': arr[i], 'value': i + 1})
            }
          }
          this.temTableData[this.clickRowIndex].original0ptional = this.desc
        }
    ...
    checkItems () {
      ...
      for (; i < this.temTableData.length; i++) {
        ...
        if (this.temTableData[i].name === '' || this.temTableData[i].type === '') {
          break
        } else if ((this.temTableData[i].type === 2 || this.temTableData[i].type === 3 || this.temTableData[i].type === 4) && this.temTableData[i].optional.length === 0) {
          break
        }
      }
