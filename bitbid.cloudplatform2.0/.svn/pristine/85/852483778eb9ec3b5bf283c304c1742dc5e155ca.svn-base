...
const mutations = {
  ...
  SET_SCOPE_OF_CONTROL_TYPE: (state, scopeOfControlType) => {
    state.scopeOfControlType = scopeOfControlType
  },
  SET_SCOPE_OF_CONTROL_DEPARTMENT_IDS: (state, scopeOfControlDepartmentIds) => {
    state.scopeOfControlDepartmentIds = scopeOfControlDepartmentIds
  },
  // 添加tabs
  add_tabs (state, data) {
...
const actions = {
  ...
  GetLoginInfo: ({commit, state}) => {
    return new Promise((resolve, reject) => {
      ...
      base.getLoginInfo(state.token).then(response => {
        ...
        // 保存控制权限范围到state中
        commit('SET_SCOPE_OF_CONTROL_TYPE', response.data.scopeOfControlType)
        // 保存控制权限组到state中
        commit('SET_SCOPE_OF_CONTROL_DEPARTMENT_IDS', response.data.scopeOfControlDepartmentIds)
        resolve()
      }).catch(error => {
        reject(error)
      })
  ...
  ClearLoginInfo: ({commit}) => {
    ...
    commit('SET_SCOPE_OF_CONTROL_TYPE', null)
    commit('SET_SCOPE_OF_CONTROL_DEPARTMENT_IDS', [])
    // 清除cookie中的token
    removeToken()
    state.openTab = []
  ...
  /** 判断权限并提示 */
  JudgePermission: ({state}, {permissionId, path}) => {
  ...
  JudgeScopeOfController: ({state}, {departmentListIds, userIds}) => {
    ...
    if (state.scopeOfControlType === 2 && departmentListIds !== null) {
