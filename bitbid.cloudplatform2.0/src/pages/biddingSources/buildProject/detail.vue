    ...
    <div class="main">
      ...
      <div class="detail">
        <div class="title">
          ...
          <el-button type="primary" @click="joinBusinessOpportunities" v-loading="loading" v-if="!isJoinOpportunity"> + 加入到我的商机</el-button>
          <el-button type="primary" @click="skipOpportunities" v-loading="loading" v-if="isJoinOpportunity"> + 查看商机详情</el-button>
        </div>
        <div class="part">
          <span>项目跟进</span>
          <div class="tablebox">
          ...
          <div class="tablebox">
            <table>
              ...
              <tr>
                ...
                <td>{{dateFormat('', '', ruleForm.plan_start_time)}}</td>
                <td class="gray">竣工时间</td>
                <td>{{dateFormat('', '', ruleForm.plan_end_time)}}</td>
              </tr>
              <tr>
...
export default {
  ...
  methods: {
    detail () {
      ...
      // 是否加入商机
      proposedProject.isJoinOpportunity(this.$route.params.code).then(res => {
    ...
    joinBusinessOpportunities () {
      ...
      this.opportunitiesForm.projectAmount = this.ruleForm.total_investment ? (this.ruleForm.total_investment_company === 1 ? (this.ruleForm.total_investment * 10000) : (this.ruleForm.total_investment_company === 2 ? (this.ruleForm.total_investment * 100000000) : this.ruleForm.total_investment)) : '0'
      this.opportunitiesForm.projectedRevenue = this.ruleForm.total_investment ? (this.ruleForm.total_investment_company === 1 ? (this.ruleForm.total_investment * 10000) : (this.ruleForm.total_investment_company === 2 ? (this.ruleForm.total_investment * 100000000) : this.ruleForm.total_investment)) : '0'
      this.opportunitiesForm.contacts = this.contactTableData.length ? this.contactTableData.filter(item => item.type === 1)[0].contact : null
      this.opportunitiesForm.contactNum = this.contactTableData.length ? this.contactTableData.filter(item => item.type === 1)[0].phone : null
    ...
    // 跳转商机详情
    skipOpportunities () {
      business.detailByComparisonCode(this.$route.params.code).then(res => {
