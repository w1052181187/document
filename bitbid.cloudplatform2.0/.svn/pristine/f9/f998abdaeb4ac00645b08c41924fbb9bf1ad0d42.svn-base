        ...
        <el-row>
          <el-col :span="24" class="ueditor_box" >
            <el-form-item label="标段内容：" prop="sectionContent" ref="sectionContent">
              <editor ref="sectionEditor" class="ueditor" :editread="editread" v-model="updateForm.sectionContent"></editor>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24" class="ueditor_box">
            <el-form-item label="投标人资格条件：" prop="bidderContent" ref="bidderContent">
              <editor ref="bidderEditor" class="ueditor" :editread="editread" v-model="updateForm.bidderContent"></editor>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      ...
      approvalTask.getByRelatedCode({
        ...
        if (res.data.resCode === '0000') {
          if (res.data.approvalTask) {
            this.tableFlag = true
            this.$nextTick(() => {
              this.$refs.sectionEditor.setContent(this.updateForm.bidSectionContent)
              this.$refs.bidderEditor.setContent(this.updateForm.bidQualification)
            })
          }
        }
      })
    ...
    getChooseBidSectionInfo () {
      bidSection.getOne(this.$route.params.code).then((res) => {
        if (res.data.bidSection) {
          ...
          // 审批信息的展示
          if (Number(this.updateForm.flowStatus) !== 1) {
