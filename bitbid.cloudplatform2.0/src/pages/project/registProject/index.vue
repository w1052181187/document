            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="项目类型：">
                  ...
                  <el-radio-group v-model="searchForm.tenderProjectTypeFirstRank" @change="conditionSelect($event, '项目类型')">
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      ...
      if (this.searchForm.tenderProjectTypeFirstRank) {
        query.tenderProjectTypeFirstRank = this.searchForm.tenderProjectTypeFirstRank
      }
      ...
      tenderProject.getList(query).then((res) => {
        if (res.data.tenderProjectList && res.data.tenderProjectList.list) {
          ...
          this.registProjectData.forEach(item => {
            ...
            if (list.length > 0) {
              item.tenderProjectTypeStr = this.filterTenderProjectType(list[0])
            } else {
    ...
    filterTenderProjectType (tenderProjectType) {
      if (Number(tenderProjectType) === 1 || tenderProjectType === 'A') {
        return '工程'
      } else if (Number(tenderProjectType) === 2 || tenderProjectType === 'B') {
        return '货物'
      } else if (Number(tenderProjectType) === 3 || tenderProjectType === 'C') {
        return '服务'
      }
    },
    ...
    reset () {
      ...
      this.searchForm.tenderProjectTypeFirstRank = ''
    ...
    // 查看
    lookBtn (scope) {
      this.$router.push({path: `/project/registProject/detail/${scope.row.code}`})
    },
    // 修改
    updateBtn (scope) {
      this.$store.dispatch('JudgeScopeOfController', {departmentListIds: scope.row.departmentListIds, userIds: [scope.row.creator]}).then(result => {
        if (result) {
          this.$router.push({path: `/project/registProject/update/${scope.row.code}`})
        }
      })
    },
    // 删除
