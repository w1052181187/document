        ...
        <ul>
          <li v-for="item in listData" :key="item.objectId">
            <div class="record">
              ...
              <span class="phase" v-if="$route.params.type === 'opportunity'">{{item.opportunityStageInfo}}</span>
              <div class="handle">
                <span title="编辑" v-if="$store.getters.authUser.userId === item.creator" @click="showEditDialog(true, true, item.objectId)"><img src="../../../../static/images/ico_edit.png"/> </span>
                <span title="删除" v-if="$store.getters.authUser.userId === item.creator" @click="logoff(item.objectId)"><img src="../../../../static/images/ico_del.png"/> </span>
