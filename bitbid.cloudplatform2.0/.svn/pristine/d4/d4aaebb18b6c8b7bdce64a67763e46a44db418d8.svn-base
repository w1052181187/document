  ...
  methods: {
    cellClick (row, column, cell, event) {
      if (column.label === '项目编号' || column.label === '项目名称') {
        sessionStorage.setItem('checkSummaryAndAndEvaluate', row.checkSummaryAndAndEvaluate)
        this.$router.push({path: `/project/projectSum/detail/${row.objectId}`, query: {tenderSystemCode: row.code}})
      }
    },
    filterTenderMode (row) {
      if (Number(row.tenderMode) === 1) {
        if (Number(row.isPreBid) === 1) {
          return '公开招标（资格预审）'
        } else {
          return '公开招标（资格后审）'
        }
    ...
    getTableData () {
      ...
      tenderProject.getListInProjectSum(query).then((res) => {
