    ...
    <div class="main">
      ...
      <div class="section_listbox">
        <div class="section_list archive-section-list">
          <ul class="section_ul">
            <li v-for="(item, index) in bidList" :key="item.index" :class="['archive-section_li', activeBid === index ? 'cur': '']" @click="handleBidClick(item, index)">
              <p :title="item.bidSectionName">{{item.bidSectionName}}</p>
              <span>归档：{{item.archivesSchedule}}</span>
              <img src="../../../../static/images/daijieshou_1.png" v-if="Number(item.isWaitTransfer) === 1 ">
            </li>
