        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="是否需要报名：" prop="isSignUp">
              <el-select v-model="updateForm.isSignUp" placeholder="请选择" @change="changeIsSignUp" :disabled="isSignUpFlag">
  ...
  data () {
    return {
      ...
      rules: {
        ...
        bidSectionList: [
          { required: false, message: '请选择标段', trigger: ['blur', 'change'] }
        ],
        content: [
        ...
        isSignUp: [
          {required: true, message: '请选择是否需要报名信息', trigger: 'blur'}
        ],
        signUpBeginTime: [
    ...
    getTenderProjectInfo () {
      tenderProject.getByBidSectionCode(this.$route.query.sectionSystemCode).then((res) => {
        if (res.data.tenderProject) {
          ...
          // 预审的项目必须报名
          if (this.updateForm.type === 2) {
            this.updateForm.isSignUp = '1'
