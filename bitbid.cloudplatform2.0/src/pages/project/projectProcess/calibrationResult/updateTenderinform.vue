  ...
  data () {
    return {
      ...
      rules: {
        ...
        noticeName: {required: true, message: '请填写招标结果通知书标题', trigger: ['blur', 'change']}
      },
      tableData: [],
  ...
  methods: {
    ...
    getResultInfo () {
      resultInfo.getById(this.$route.query.objectId).then(res => {
        ...
        if (this.updateForm.successfulBidders && this.updateForm.successfulBidders.length > 0) {
          let bidderId = this.updateForm.bidderId.split(';')
          let successfulBidders = this.updateForm.successfulBidders.split(';')
          for (let i = 0; i < successfulBidders.length; i++) {
            let result = {}
            result.bidderId = bidderId[i]
            result.bidderName = successfulBidders[i]
            this.updateForm.successfulBidderList.push(result)
            this.excludeNames += ',' + successfulBidders[i]
          }
        }
        if (this.updateForm.unsuccessfulBidders && this.updateForm.unsuccessfulBidders.length > 0) {
          let unsuccessfulBidderIds = this.updateForm.unsuccessfulBidderIds.split(';')
          let unsuccessfulBidders = this.updateForm.unsuccessfulBidders.split(';')
          for (let i = 0; i < unsuccessfulBidders.length; i++) {
            let result = {}
            result.bidderId = unsuccessfulBidderIds[i]
