<template>
  <div class="custom-charge-list cloudcontent" id="tenderFeeList" :loading="loading">
    ...
    <div class="main">
      <div class="fr" v-if="Number(isToSettle) === 0 && $store.getters.permissions.includes('1050402')">
      ...
      <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="modifyRowLine(scope.row)" v-if="(Number(scope.row.status) === 0 && Number(isToSettle) === 0) && $store.getters.permissions.includes('1050402')">修改</el-button>
            <el-button type="text" size="small" @click="modifyRowLine(scope.row)" v-if="(Number(scope.row.status) === 1 && Number(isToSettle) === 0) && $store.getters.permissions.includes('1050402')">更正收费</el-button>
            <el-button type="text" size="small" @click="returnRowLine(scope.row)" v-if="(Number(scope.row.status) === 2 && Number(isToSettle) === 0) && $store.getters.permissions.includes('1050402')">更正退还</el-button>
            <el-button type="text" size="small" @click="returnRowLine(scope.row)" v-if="(Number(scope.row.status) === 1 && Number(isToSettle) === 0) && $store.getters.permissions.includes('1050402')">退还</el-button>
            <el-button type="text" size="small" @click="seeRowDetail(scope.row)">查看</el-button>
            <el-button type="text" size="small" @click="delRowLine(scope.row)" v-if="(Number(scope.row.status) === 0 && Number(isToSettle) === 0) && $store.getters.permissions.includes('1050402')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
