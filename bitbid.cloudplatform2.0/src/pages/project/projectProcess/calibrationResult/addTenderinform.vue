<template>
  <div class="cloudcontent" id="cloud_processedit">
    ...
    <section-bidder-dialog v-if="tradingPlatformFlag" :tenderSystemCode="$route.query.tenderSystemCode" :excludeNames="excludeNames" :includeNames="includeNames"
          ...
          data.map(item => {
            if (item.candidateList) {
              item.candidateList.map(candidate => {
                ...
                if (this.updateForm.bidderId) {
                  this.updateForm.bidderId += ';' + candidate.bidderId
                } else {
                  this.updateForm.bidderId += candidate.bidderId
                }
                if (this.updateForm.successfulBidders) {
        ...
        if (data) {
          ...
          data.map(candidate => {
            if (!bidderIds.includes(candidate.bidderId.toString())) {
              ...
              if (this.updateForm.unsuccessfulBidderIds) {
                this.updateForm.unsuccessfulBidderIds += ';' + candidate.bidderId
              } else {
                this.updateForm.unsuccessfulBidderIds += candidate.bidderId
              }
              if (this.updateForm.unsuccessfulBidders) {
