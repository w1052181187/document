      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="logoff(scope)" v-if="$store.getters.permissions.includes('1040102')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
...
export default {
  ...
  methods: {
    ...
    // 查看
    clickTable (row) {
      this.$router.push({path: `/customerMan/customer/detail/${row.code}`})
    },
    // 修改
    updateBtn (scope) {
      ...
      this.$store.dispatch('JudgeScopeOfController', {departmentListIds: scope.row.departmentListIds, userIds: [scope.row.creator]}).then(result => {
        if (result) {
          this.$confirm('确认删除吗?', '提示', {
            ...
            customer.logoff(scope.row.code).then(res => {
