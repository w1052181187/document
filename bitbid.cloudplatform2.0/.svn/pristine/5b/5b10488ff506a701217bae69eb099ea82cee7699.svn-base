    ...
    <el-dialog title="选择投标人" width="600px" :before-close="cancel" :visible.sync="showVisible">
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="新建投标人" name="second"
                     v-if="!tradingPlatformFlag && flowStatus !== 9 && flowStatus !== 10 && flowStatus !== 12 &&
                      flowStatus !== 13 && flowStatus !== 15 && flowStatus !== 19 && flowStatus !== 20">
...
export default {
  ...
  props: ['showVisible', 'selectedCode', 'tenderSystemCode', 'excludeNames', 'includeNames', 'flowStatus', 'flag'],
  watch: {
  ...
  methods: {
    ...
    cancel () {
      ...
      this.addForm = {}
      if (!this.tradingPlatformFlag && this.flowStatus !== 9 && this.flowStatus !== 10 && this.flowStatus !== 12 &&
        this.flowStatus !== 13 && this.flowStatus !== 15 && this.flowStatus !== 19 && this.flowStatus !== 20) {
      ...
      this.$refs[form].validate((vaild) => {
        if (vaild) {
          ...
          bidder.saveBidder(this.addForm).then(res => {
            this.addForm.objectId = res.data.bidder.objectId
