<template>
  <div class="cloudcontent" id="cloud_processedit">
    <div class="main">
      ...
      <bidder-dialog v-if="!tradingPlatformFlag" :tenderSystemCode="updateForm.tenderSystemCode" :excludeNames="excludeBidderNames" :includeNames="includeBidderNames"
                     :flowStatus="flowStatus" :showVisible="showBidderVisible" @selected="selectedBidder" @showDialog="showBidderDialog"></bidder-dialog>
      <section-bidder-dialog v-if="tradingPlatformFlag" :tenderSystemCode="updateForm.tenderSystemCode" :excludeNames="excludeBidderNames" :includeNames="includeBidderNames"
                             :flowStatus="flowStatus" :showVisible="showBidderVisible" @selected="selectedBidder" @showDialog="showBidderDialog" :sectionSystemCode="updateForm.sectionSystemCode"></section-bidder-dialog>
    </div>
  </div>
</template>
<script>
...
import {documentInfo, resultInfo} from '@/api/project'
  ...
  methods: {
    ...
    getDocumentInfo () {
      documentInfo.getById(this.$route.query.objectId).then(res => {
        ...
        this.updateForm.winCandidateList.map(item => {
    ...
    selectedBidder (obj) {
      ...
      this.dialogForm.tenderSystemCode = this.updateForm.tenderSystemCode
      this.dialogForm.sectionSystemCode = this.updateForm.sectionSystemCode
    },
    getFileInfoList () {
      let query = {
      ...
      resultInfo.getList(query).then(res => {
        ...
        if (data) {
          data.map(resultInfo => {
            this.includeBidderNames.push(resultInfo.bidderName)
          })
        }
      })
    }
