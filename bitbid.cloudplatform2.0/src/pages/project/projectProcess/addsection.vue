<template>
  <div class="cloudcontent" id="cloud_addsection">
    <div class="main">
      ...
      <submitApproveDialog :flowStatus="flowStatus" :tenderSystemCode="$route.query.code" :showVisible="approveDialogVisable" ref="approvalDialog" @confirmSubmit="confirmSubmit" @handleCancel="cancelApprove"></submitApproveDialog>
    </div>
  </div>
</template>
<script>
    ...
    getChooseTenderProjectInfo () {
      tenderProject.getBaseByCode(this.$route.query.code).then((res) => {
        if (res.data.tenderProject) {
          ...
          // 五期需求：保存招标方式和资格预审方式
          this.updateForm.tenderMode = res.data.tenderProject.tenderMode
          this.updateForm.isPreBid = res.data.tenderProject.isPreBid
        }
      })
    },
    /** 校验富文本框内容 */
