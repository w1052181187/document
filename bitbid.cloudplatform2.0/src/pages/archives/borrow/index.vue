          ...
          <el-form :model="searchForm" ref="searchForm" label-width="82px" :validate-on-rule-change="true">
            <el-row>
              ...
              <el-date-picker
                ...
                v-if="searchForm.borrowingTimeType === 6"
                type="daterange"
                range-separator="至"
            ...
            <el-row>
              <el-col :span="9" class="bigWidth">
                <el-form-item label="所属部门：">
                  <el-input class="deptInput" v-model="searchForm.searchDepartmentName" auto-complete="off" placeholder="请选择" readonly="readonly" clearable @click.native="showDepartmentTree">
                    <i slot="suffix" class="el-input__icon el-icon-arrow-down"></i>
                  </el-input>
                  <tree-options :initData="searchForm.borrowerDepartmentId" @selectSuccess="selectedDepatment" :isShowOptions="isShowDptTree" :startWithRootFlag = "true"></tree-options>
                </el-form-item>
              </el-col>
              <el-col :span="9" class="bigWidth">
    ...
    <div class="main">
      <el-table
        ...
        <el-table-column
          ...
          width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.borrowerStatus === 1">待借出</span>
            <span v-if="scope.row.borrowerStatus === 2">已借出</span>
            <span v-if="scope.row.borrowerStatus === 3">已归还</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="borrowingDate"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="handleBorrow(scope.row, 'borrow')" v-if="scope.row.borrowerStatus === 1 && $store.getters.permissions.includes('1070502')">借出</el-button>
            <el-button type="text" size="small" @click="handleBorrow(scope.row, 'return' )"  v-if="scope.row.borrowerStatus === 2 && $store.getters.permissions.includes('1070502')">归还</el-button>
...
export default {
  ...
  methods: {
    ...
    conditionSelect (value, label) {
      switch (label) {
        case '档案状态':
          this.searchForm.borrowerStatus = value
      ...
      switch (label) {
        ...
        case '借阅时间':
          if (!this.isBorrowTimeSelectAll) {
            ...
            this.searchForm.borrowingTimeType = null
    ...
    selectedDepatment (deptTreeNode) {
      this.searchForm.searchDepartmentName = deptTreeNode.label
      this.searchForm.borrowerDepartmentId = deptTreeNode.id
    ...
    handleDetail (row) {
      this.$router.push({path: `/archives/borrow/detail/${row.journalSummaryId}`, query: {isApproved: 3, auditStatus: 1}})
    },
    handleBorrow (row, type) {
      this.$confirm(type === 'borrow' ? '您确定要将该档案借出吗' : '请再次确认该档案已经归还?', '提示', {
        ...
        switch (type) {
          case 'borrow':
            archivesProject.lendByObjectId(row.objectId).then((res) => {
              if (res.data.resCode === '0000') {
                this.getData()
              }
            })
            break
          case 'return':
            archivesProject.returnByObjectId(row.objectId).then((res) => {
              if (res.data.resCode === '0000') {
                this.getData()
              }
            })
