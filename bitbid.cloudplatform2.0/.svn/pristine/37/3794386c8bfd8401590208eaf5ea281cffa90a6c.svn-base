<template>
  <div class="cloudcontent" id="cloud_processedit">
    <div class="main">
      ...
      <submitApproveDialog :flowStatus="flowStatus" :tenderSystemCode="$route.query.tenderSystemCode" ref="approvalDialog" :showVisible="approveDialogVisable" @confirmSubmit="confirmSubmit" @handleCancel="cancelApprove"></submitApproveDialog>
      <el-dialog
        :title="isEdit ? '编辑' : '新增'"
      ...
      <section-bidder-dialog v-if="tradingPlatformFlag" :tenderSystemCode="updateForm.tenderSystemCode" :flowStatus="flowStatus" :includeNames="includeNames" :showVisible="showBidderVisible" @selected="selectedBidder" @showDialog="showBidderDialog" :sectionSystemCode="$route.query.sectionSystemCode"></section-bidder-dialog>
    </div>
  </div>
</template>
<script>
...
import {documentInfo, bidSection, tenderProject, resultInfo} from '@/api/project'
...
export default {
  ...
  methods: {
    ...
    submitData (status) {
      ...
      this.$confirm(msg, '提示', {
        ...
        this.updateForm.bidderList.map(item => {
          bidderId = bidderId + item.bidderId + ';'
          this.updateForm.bidOpeningParticipantList.push(item)
        })
        this.updateForm.agencyList.map(item => {
          this.updateForm.bidOpeningParticipantList.push(item)
        })
        this.updateForm.bidderId = bidderId
    ...
    getTenderProjecct () {
      tenderProject.getBaseByCode(this.$route.query.tenderSystemCode).then(res => {
