  ...
  <div class="handle-box">
    <div class="handle">
      ...
      <template v-if="Number(isApproved) === 3">
        ...
        <el-button v-if="Number($route.query.auditStatus) === 0" type="primary" class="back" @click="handleRecall">撤回</el-button>
      </template>
  ...
  methods: {
    ...
    handleUrge () {
      this.$confirm('确定要发起催办吗?', '提示', {
        ...
        approvalTask.urgeApproval(this.approvalTaskCode).then((res) => {
          if (res.data.resCode === '0000') {
            this.handleCancle()
          }
        })
