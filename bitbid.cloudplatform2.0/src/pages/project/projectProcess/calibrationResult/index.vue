  ...
  <div class="cloudcontent" id="cloud_processtype">
    <div class="box">
      <div class="topmain">
        中标候选人公示
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="addWinbidpublicBtn()" v-if="operationFlag && publicTable.length === 0">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="publishWinbidpublicBtn(scope)" v-if="scope.row.auditStatus === 2 || scope.row.auditStatus === 4">发布</el-button>
    ...
    <div class="box">
      <div class="topmain">
        中标候选人公示疑义及澄清
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="addWinbidclearBtn()" v-if="operationFlag">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 0 || scope.row.auditStatus === 3 || scope.row.auditStatus === 5) && operationFlag" @click="updateWinbidclearBtn(scope)">编辑</el-button>
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 0) && operationFlag" @click="delWinbidclearBtn(scope)">删除</el-button>
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 4 && scope.row.clarificationState === 0) && operationFlag" @click="clearWinbidclearBtn(scope)">澄清</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
    ...
    <div class="box">
      <div class="topmain">
        中标确认函
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="addWinbidOkBtn()" v-if="operationFlag && winbidOkData.length === 0">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      <el-table
        ...
        <el-table-column
          prop="changeNums"
          label="更改次数"
          :formatter="filterChangeNums"
          width="120">
        </el-table-column>
        <el-table-column
          label="操作" align="center" width="260">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="lookWinbidOkBtn(scope)">查看</el-button>
            <el-button type="text" size="small" v-if="(scope.row.status === 0 || scope.row.status === 3 || scope.row.status === 5) && operationFlag" @click="updateWinbidOkBtn(scope)">编辑</el-button>
            <el-button type="text" size="small" v-if="scope.row.status === 0 && operationFlag" @click="delWinbidOkBtn(scope)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
    ...
    <div class="box">
      <div class="topmain">
        中标结果公告
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="addWinbidnoticeBtn" v-if="operationFlag && winbidNoticeTable.length === 0">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="publishWinbidnoticeBtn(scope)" v-if="scope.row.auditStatus === 2 || scope.row.auditStatus === 4">发布</el-button>
    ...
    <div class="box">
      ...
      <el-table
        ...
        <el-table-column
          ...
          width="120">
          <template slot-scope="scope">
            <span  v-if="scope.row.clarificationState === 0">未回复</span>
            <span  v-if="scope.row.clarificationState === 1">已回复</span>
            <span  v-if="scope.row.clarificationState !== 0 && scope.row.clarificationState !== 1">---</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="clarificationTime"
        ...
        <el-table-column
          label="操作" align="center" width="260">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="replyWinbidnoticeObjectionBtn(scope)" v-if="operationFlag && scope.row.clarificationState === 0 && (scope.row.auditStatus === 2 || scope.row.auditStatus === 4)">回复</el-button>
            <el-button type="text" size="small" @click="lookWinbidnoticeObjectionBtn(scope)">查看</el-button>
            <el-button type="text" size="small" @click="updateWinbidnoticeObjectionBtn(scope)" v-if="operationFlag && !checkDataSource && (scope.row.auditStatus === 0 || scope.row.auditStatus === 3)">编辑</el-button>
            <el-button type="text" size="small" @click="delWinbidnoticeObjectionBtn(scope)" v-if="operationFlag && !checkDataSource && scope.row.auditStatus === 0">删除</el-button>
            </template>
        </el-table-column>
      </el-table>
      <!--分页-->
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 2 || scope.row.auditStatus === 4) && operationFlag" @click="publishWinbidinformBtn(scope)">发出</el-button>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 2 || scope.row.auditStatus === 4) && operationFlag" @click="publishTenderinformBtn(scope)">发出</el-button>
    ...
    <div class="box">
      ...
      <el-table
        ...
        <el-table-column
          ...
          width="120">
          <template slot-scope="scope">
            <span  v-if="scope.row.refundAmount && scope.row.auditStatus === 4">已退还</span>
            <span  v-else>----</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="scope.row.auditStatus === 0 || scope.row.auditStatus === 3 || scope.row.auditStatus === 5" @click="updateReturnBondBtn(scope)">退还</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
...
<script>
import {bulletinInfo, questionAndAnswerInfo, resultInfo, costInfo, winBiddingConfirmationLetter} from '@/api/project/'
import {dateFormat} from '@/assets/js/common'
import {confirmSuccess} from '@/assets/js/utils'
export default {
  components: {
  },
  ...
  methods: {
    ...
    /** 中标确认函枚举 */
    filterChangeNums (row, col, cellValue) {
    ...
    publishWinbidpublicBtn (scope) {
      confirmSuccess(this, '确认发布吗?', (cancelLoading) => {
    ...
    lookWinbidOkBtn (scope) {
      this.$router.push({path: `/project/projectProcess/project/process/calibrationResult/winbidOk/detail/${scope.row.code}`, query: {sectionSystemCode: this.sectionSystemCode}})
    },
    // 中标确认函-编辑
    updateWinbidOkBtn (scope) {
      this.$router.push({path: `/project/projectProcess/project/process/calibrationResult/winbidOk/add`, query: {code: scope.row.code}})
    },
    // 中标确认函-删除
    delWinbidOkBtn (scope) {
      this.$confirm('确认删除吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error'
      }).then(() => {
        winBiddingConfirmationLetter.delete(scope.row.objectId).then((res) => {
          if (res.data.resCode === '0000') {
            this.getLetterList()
          }
        })
    ...
    publishWinbidnoticeBtn (scope) {
      confirmSuccess(this, '确认发出吗?', (cancelLoading) => {
        bulletinInfo.releaseBulletin(scope.row.code).then((res) => {
          cancelLoading()
          if (res.data.resCode === '0000') {
            this.getNoticeList()
    ...
    // 中标结果公告质疑及回复
    getWinbidNoticeObjectionTableList () {
      let query = {
        pageNo: this.noticeObjectionPageNo,
        pageSize: this.noticeObjectionPageSize,
        tenderSystemCode: this.tenderSystemCode,
        sectionSystemCode: this.sectionSystemCode,
        // enterpriseId: this.$store.getters.authUser.enterpriseId,
        type: 8
      ...
      questionAndAnswerInfo.getList(query).then(res => {
        let data = res.data.questionAndAnswerInfoList
        if (data) {
          this.winbidNoticeObjectionTable = data.list
          this.noticeObjectionTotal = data.total
        }
        this.$emit('checkFlowStatusFlag', this.type)
      })
    },
    ...
    lookWinbidnoticeObjectionBtn (scope) {
      this.$router.push({path: `/project/projectProcess/project/process/calibrationResult/winbidnoticeObjection/detail/${scope.row.objectId}`, query: {sectionSystemCode: this.sectionSystemCode}})
    },
    // 编辑
    updateWinbidnoticeObjectionBtn (scope) {
      this.$router.push({path: `/project/projectProcess/project/process/calibrationResult/winbidnoticeObjection/add`,
        query: {objectId: scope.row.objectId, tenderSystemCode: this.tenderSystemCode, sectionSystemCode: this.sectionSystemCode}})
    },
    // 删除
    delWinbidnoticeObjectionBtn (scope) {
      this.$confirm('确认删除吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error'
        ...
        questionAndAnswerInfo.deleteById(scope.row.objectId).then((res) => {
          if (res.data.resCode === '0000') {
            this.getWinbidNoticeObjectionTableList()
          }
        })
      }).catch(() => {
        return false
    ...
    replyWinbidnoticeObjectionBtn (scope) {
      this.$router.push({path: `/project/projectProcess/project/process/calibrationResult/winbidnoticeObjection/reply/${scope.row.objectId}`})
    },
    // 添加
    ...
    publishWinbidinformBtn (scope) {
      confirmSuccess(this, '确认发出吗?', (cancelLoading) => {
        resultInfo.gdIssue(scope.row.objectId).then(() => {
          cancelLoading()
    ...
    getLetterList () {
      let query = {
        pageNo: this.pageNoWinbidOk,
        pageSize: this.pageSizeWinbidOk,
        tenderSystemCode: this.tenderSystemCode,
        sectionSystemCode: this.sectionSystemCode
      }
      winBiddingConfirmationLetter.getList(query).then(res => {
        ...
        if (data) {
          this.winbidOkData = data.list
          this.totalWinbidOk = data.total
        }
        this.$emit('checkFlowStatusFlag', this.type)
      })
    },
