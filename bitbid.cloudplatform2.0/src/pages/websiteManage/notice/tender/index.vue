    ...
    <div class="main">
      <div class="topmain">
        ...
        <div class="seacher_box searchselect_box">
          ...
          <el-input class="search" placeholder="请输入招标名称" style="vertical-align: top"  v-model="search">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="scope.row.status === 0">添加中标公示</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
...
export default {
  ...
  methods: {
    // 添加
    addBtn () {
      this.$router.push({path: '/websiteManage/notice/tender/add'})
    },
    // 修改
    updateBtn (scope) {
      this.$router.push({path: `/websiteManage/notice/tender/update`, query: {objectId: scope.row.objectId}})
    },
    // 查看
    lookBtn (scope) {
      this.$router.push({path: `/websiteManage/notice/tender/detail/${scope.row.objectId}`})
    },
    // 分页
    handleCurrentChange () {},
    // 数据为空时显示 ---
