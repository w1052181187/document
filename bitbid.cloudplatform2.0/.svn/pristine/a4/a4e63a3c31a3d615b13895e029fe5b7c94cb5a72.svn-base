...
export default {
  ...
  methods: {
    ...
    /** 招标方式(1.公开招标；2.邀请招标；3.竞争性谈判；4.单一来源采购；5.询价采购；6.竞争性磋商；9.其他) */
    filterTenderMode (row) {
      if (row.tenderMode === 1) {
        return '公开招标'
      } else if (row.tenderMode === 2) {
        return '邀请招标'
      } else if (row.tenderMode === 3) {
        return '竞争性谈判'
      } else if (row.tenderMode === 4) {
        return '单一来源采购'
      } else if (row.tenderMode === 5) {
        return '询价采购'
      } else if (row.tenderMode === 6) {
        return '竞争性磋商'
      } else if (row.tenderMode === 9) {
        return '其他'
      }
    },
    /** 获取列表数据 */
