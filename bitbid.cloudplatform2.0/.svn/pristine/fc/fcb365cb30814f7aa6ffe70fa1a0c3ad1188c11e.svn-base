...
import {tenderProject, bidSection, settlementGroup} from '@/api/project'
...
export default {
  ...
  methods: {
    ...
    // 获取未退还的投标人数量
    getUnReturnPeopleCount () {
      settlementGroup.getUnReturnPeopleCount({}).then((res) => {
        if (res.data.resCode === '0000') {
          this.unReturnPeopleCount = res.data.unReturnPeopleCount
        }
      })
    },
    // 获取未读公告
