...
(function($) {
    ...
    $.extend(Plugin.prototype, {
        ...
        createSvg: function() {
          let half = this.settings.dimension / 2;
          let quarter = this.settings.dimension / 4;
          let area = Math.PI * 2 * quarter;
          let svg =
        ...
        animateNumber: function() {
            let $target = $(this.element).find('.percentage');
        ...
        animateStrokeDasharray: function() {
            let debug = this.settings.percentage >= 100 ? 1 : 0; // to debug non webkit browsers
            let area = 2 * Math.PI * ((this.settings.dimension / 4) + 0.4); // +0.4 to debug non webkit browsers
            let strokeEndValue = (this.settings.percentage + debug) * area / 100;
            let $target = $(this.element).find('svg circle:nth-child(2)');
