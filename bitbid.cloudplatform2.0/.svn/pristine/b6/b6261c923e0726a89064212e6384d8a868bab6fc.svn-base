  ...
  <div class="calendar">
    <div class="calendar-sonbox">
      ...
      <span class="calendar-prev" @click="prev">
        <svg width="15" height="15" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
          <g class="transform-group">
            <g transform="scale(0.015625, 0.015625)">
              <path d="M671.968 912c-12.288 0-24.576-4.672-33.952-14.048L286.048 545.984c-18.752-18.72-18.752-49.12 0-67.872l351.968-352c18.752-18.752 49.12-18.752 67.872 0 18.752 18.72 18.752 49.12 0 67.872l-318.016 318.048 318.016 318.016c18.752 18.752 18.752 49.12 0 67.872C696.544 907.328 684.256 912 671.968 912z" fill="#0e8cf0">
              </path>
            </g>
          </g>
        </svg>
      ...
      <tbody>
        <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
          <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
            ...
            <div  class="text" v-if="lunar"
              :class="{'isLunarFestival' :child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}">
              {{child.lunar}}
            </div>
            <div class="day-count" v-if="child.eventsValue">
              <el-tooltip :content="'公告数量：' + child.eventsValue" placement="top">
                <el-button class="day-count-btn">{{child.eventsValue}}</el-button>
              </el-tooltip>
            </div>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    ...
    render (y, m) {
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (this.range) { // 范围
          ...
          } else if (chkY === this.year && chkM === this.month && i === this.day && this.value.length === 0) { // 没有默认值的时候显示选中今天日期
            temp[line].push(Object.assign(
              {day: i, selected: true},
              this.getLunarInfo(this.year, this.month + 1, i),
              this.getEvents(this.year, this.month + 1, i)
    ...
    getEvents (y, m, d) {
      ...
      let eventsKey = dateFormat(new Date(y, m - 1, d), 'yyyy-MM-dd')
      let eventsValue = this.events[eventsKey]
      if (eventsKey !== undefined && eventsValue !== undefined) {
        data.eventsKey = eventsKey
        data.eventsValue = eventsValue
        data.clickFlag = true
      }
      return data
    },
    ...
    select (k1, k2, e) {
      ...
      if (this.range) {
        ...
        this.today = [k1, k2]
        this.$emit('select', [this.year, this.zero ? this.zeroPad(this.month + 1) : this.month + 1, this.zero ? this.zeroPad(this.days[k1][k2].day) : this.days[k1][k2].day], this.days[k1][k2].eventsValue ? 1 : 0)
      }
    },
    changeYear () {
      if (this.yearsShow) {
      ...
      this.yearsShow = true
      this.years = []
      for (let i = ~~this.year - 1; i < ~~this.year + 2; i++) {
        this.years.push(i)
      }
    },
    selectYear (value) {
