    ...
    <div class="main">
      <el-form :model="addForm" :rules="rulesList" ref="addForm" :validate-on-rule-change="true">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="是否PPP项目：" prop="isPppProject">
              <el-select v-model="addForm.isPppProject" placeholder="请选择">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="详细地址："  prop="tenderExpandsInfo.address">
              <el-input v-model="addForm.tenderExpandsInfo.address"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="联系人："  prop="tenderExpandsInfo.contactName">
              <el-input v-model="addForm.tenderExpandsInfo.contactName"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="联系方式："  prop="tenderExpandsInfo.contactNumber">
              <el-input v-model="addForm.tenderExpandsInfo.contactNumber"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="邮箱：" prop="tenderExpandsInfo.contactEmail">
              <el-input v-model="addForm.tenderExpandsInfo.contactEmail"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <submitApproveDialog :flowStatus="flowStatus" :showVisible="approveDialogVisable" :tenderSystemCode="addForm.code" ref="approvalDialog" @confirmSubmit="confirmSubmit" @handleCancel="cancelApprove"></submitApproveDialog>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      addForm: {
        ...
        selectedIndustryOptions: null,
        tenderProjectTypeList: null
      },
      rules: {
        tenderProjectName: [
        ...
        'tenderExpandsInfo.contactName': [
          { required: true, message: '联系人名称不能为空', trigger: 'blur' }
        ],
        'tenderExpandsInfo.contactNumber': [
          { required: true, message: '联系方式不能为空', trigger: 'blur' },
          {validator: validPhoneUser, trigger: 'blur'}
        ],
        'tenderExpandsInfo.contactEmail': [
        ...
        departmentName: [
          { required: true, message: '请选择项目所属部门', trigger: ['blur', 'change'] }
        ],
        isPppProject: [
          { required: true, message: '请选择是否为PPP项目', trigger: 'blur' }
        ],
        tenderProjectTypeList: [
          { required: true, message: '请选择项目类型', trigger: ['blur', 'change'] }
        ],
        'tenderExpandsInfo.address': [
      ...
      saveRules: {
        ...
        isPppProject: [
          { required: false, message: '请选择是否为PPP项目', trigger: ['blur', 'change'] }
        ],
        tenderProjectTypeList: [
          { required: false, message: '请选择项目类型', trigger: ['blur', 'change'] }
        ],
        'tenderExpandsInfo.address': [
          {required: false, message: '详细地址不能为空', trigger: ['blur', 'change']}
        ]
      },
      // 地址三级联动
  ...
  methods: {
    ...
    /** 查看代理合同信息 */
    lookBtn (scope) {
      this.$router.push({path: `/knowledge/agencyContract/detail/${scope.row.objectId}`, query: {code: scope.row.code}})
    },
    /** 获取当前招标项目信息 */
      ...
      tenderProject.getOne(this.$route.params.code).then((res) => {
        if (res.data.tenderProject) {
          if (localStorage.getItem(this.$route.fullPath)) {
            this.addForm = JSON.parse(localStorage.getItem(this.$route.fullPath))
          } else {
            this.addForm = res.data.tenderProject
          }
          this.addForm.isPppProject = this.addForm.isPppProject !== null ? this.addForm.isPppProject.toString() : this.addForm.isPppProject
          // 获取项目扩展信息
          if (this.addForm.tenderExpandsInfo) {
            this.addForm.tenderExpandsInfo.objectId = null
            if (this.addForm.tenderExpandsInfo.provinceId) {
              ...
              this.addForm.selectedCityOptions = null
            }
            if (this.addForm.tenderExpandsInfo.industryTypeFirst) {
              ...
              this.addForm.selectedIndustryOptions = null
            }
          } else {
          ...
          if (this.addForm.tenderProjectType) {
            ...
            this.addForm.tenderProjectTypeList = null
          }
          // 获取文本编辑器的内容
    ...
    confirmSubmit (record) {
      ...
      if (record.noticeList && record.noticeList.length > 0) {
        this.addForm.noticeList = record.noticeList
      }
    ...
    handleContentChange () {
      if (this.$refs.ueditor) {
        this.addForm.projectContent = this.$refs.ueditor.getContent()
        localStorage.setItem(this.$route.fullPath, JSON.stringify(this.addForm))
      }
    }
  },
  watch: {
