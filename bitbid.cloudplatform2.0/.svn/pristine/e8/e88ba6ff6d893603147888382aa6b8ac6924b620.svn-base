<template>
  <div class="cloudcontent">
    ...
    <bidder-dialog v-if="!tradingPlatformFlag" :tenderSystemCode="tenderSystemCode" :showVisible="showBidderVisible" :includeNames="includeBidderNames"
                   :flowStatus="flowStatus" @selected="selectedBidder" @showDialog="showBidderDialog"></bidder-dialog>
    <section-bidder-dialog v-if="tradingPlatformFlag" :tenderSystemCode="$route.query.tenderSystemCode" :includeNames="includeBidderNames"
  ...
  methods: {
    ...
    submitData (status) {
      ...
      this.$confirm(msg, '提示', {
        ...
        this.updateForm.bidderId = bidderId.substring(0, bidderId.length - 1)
    ...
    getDocumentInfo () {
      documentInfo.getById(this.$route.query.objectId).then(res => {
        ...
        let bidderId = this.updateForm.bidderId.split(';')
    ...
    selectedBidder (obj) {
      ...
      this.$set(this.resultList[this.index], 'bidderId', obj.objectId)
    },
    getResultInfoList () {
      let query = {
        tenderSystemCode: this.tenderSystemCode,
        sectionSystemCode: this.bidSection.code,
        type: 1
      }
