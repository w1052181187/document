  ...
  <div class="cloudcontent" id="cloud_projectProcess">
    <div class="main">
      <el-form :model="updateForm" :rules="rulesList" ref="updateForm" :validate-on-rule-change="true">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="投标人征集方式：" prop="biddingMethod">
              <el-select v-model="updateForm.biddingMethod">
                <el-option label="公开" value="0">公开</el-option>
                <el-option label="邀请" value="1">邀请</el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="采购组织形式：" prop="purchasingOrganizationForm">
              <el-select v-model="updateForm.purchasingOrganizationForm">
                <el-option label="政府采购" value="1">政府采购</el-option>
                <el-option label="自行采购" value="0">自行采购</el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="评标方法：" prop="bidEvaluationMethod">
              <el-input v-model="updateForm.bidEvaluationMethod"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="批准单位：">
              <el-input v-model="updateForm.approvalUnit"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="批文名称：">
              <el-input v-model="updateForm.approvalName"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="招标方式："  prop="tenderMode">
              <el-select v-model="updateForm.tenderMode" placeholder="请选择" @change="changeTenderMode">
                <el-option
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="招标人账号：" prop="purchasingAgent" v-if="checkDataSource">
              <el-input v-model="updateForm.purchasingAgent">
                <el-button type="primary" slot="append" @click="checkPurchasingAgentInfo">检查</el-button>
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <submitApproveDialog :flowStatus="flowStatus" :tenderSystemCode="$route.query.code" ref="approvalDialog" :showVisible="approveDialogVisable" @confirmSubmit="confirmSubmit" @handleCancel="cancelApprove"></submitApproveDialog>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        biddingMethod: [
          { required: true, message: '请选择投标人征集方式', trigger: 'blur' }
        ],
        purchasingOrganizationForm: [
          { required: true, message: '请选择采购组织形式', trigger: 'blur' }
        ],
        bidEvaluationMethod: [
          { required: true, message: '请输入评标方法', trigger: 'blur' }
        ],
        purchasingAgent: [
          { required: true, message: '请输入招标人账号', trigger: 'blur' }
        ]
      },
      saveRules: {
        tenderProjectName: [
        ...
        biddingMethod: [
          { required: false, message: '请选择投标人征集方式', trigger: 'blur' }
        ],
        purchasingOrganizationForm: [
          { required: false, message: '请选择采购组织形式', trigger: 'blur' }
        ],
        bidEvaluationMethod: [
          { required: false, message: '请输入评标方法', trigger: 'blur' }
        ],
        purchasingAgent: [
          { required: false, message: '请输入招标人账号', trigger: 'blur' }
        ]
      },
      // 项目类型
      options: [{
  ...
  methods: {
    /** 修改招标项目类型 */
    changeTenderMode () {
      this.updateForm.isPreBid = ''
    },
    /** 招标人信息检测 */
    checkPurchasingAgentInfo () {
      let flag = true
      if (this.updateForm.purchasingAgent && (this.updateForm.purchasingAgent !== '' || this.updateForm.purchasingAgent !== null)) {
        tenderProject.checkPurchasingAgent(this.updateForm.purchasingAgent).then((res) => {
          if (res.data.purchasingAgentId === null || res.data.purchasingAgentId === '') {
            this.$message({
              message: res.data.purchasingAgentMsg,
              type: 'warning'
            })
            flag = false
          } else {
            if (res.data.purchasingAgentFlag === 'yes') {
              this.$message({
                message: '检查成功！',
                type: 'success'
              })
              this.updateForm.purchasingAgentId = res.data.purchasingAgentId
              flag = true
            } else {
              this.$message({
                message: res.data.purchasingAgentMsg,
        ...
        if (res.data.tenderProject) {
          ...
          if (res.data.fundSourceList.length > 0) {
            ...
            this.updateForm.fundSourceList.forEach(item => {
              ...
              if (item.sourceOfFunds === 5 || item.sourceOfFunds === 6 || item.sourceOfFunds === 7 || item.sourceOfFunds === 8) {
                this.fundSourceOptions[Number(item.sourceOfFunds) - 5].disabled = true
              } else if (item.sourceOfFunds === 4) {
                this.fundSourceOptions[this.fundSourceOptions.length - 1].disabled = true
              }
            })
          } else {
          ...
          this.updateForm.biddingMethod = this.updateForm.biddingMethod !== null ? this.updateForm.biddingMethod.toString() : this.updateForm.biddingMethod
          this.updateForm.purchasingOrganizationForm = this.updateForm.purchasingOrganizationForm !== null ? this.updateForm.purchasingOrganizationForm.toString() : this.updateForm.purchasingOrganizationForm
        }
      })
    },
    /** 判断资金来源是否可选 */
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              if (Object.is(type, 'submit')) {
                ...
                /* let a = this.checkPurchasingAgentInfo()
                console.log(a)
                console.log((this.checkDataSource ? (a ? 0 : 1) : 0))
                if ((this.checkDataSource ? (a ? 0 : 1) : 0) === 0) {
                ...
                // 保存操作时，招标人账号存在时验证招标人账号信息
                if (this.updateForm.purchasingAgent && (this.updateForm.purchasingAgent !== '' || this.updateForm.purchasingAgent !== null)) {
                  tenderProject.checkPurchasingAgent(this.updateForm.purchasingAgent).then((res) => {
                    if (res.data.purchasingAgentId === null || res.data.purchasingAgentId === '') {
                      this.$message({
                        message: '招标人信息不存在，请核对！',
                        type: 'warning'
                      })
                      return false
                    }
    ...
    submitData (type) {
      ...
      this.$confirm(msg, '提示', {
        ...
        if (Object.is(type, 'submit')) {
          ...
          if (this.updateForm.fundSourceList.length === 1 && this.updateForm.fundSourceList[0].sourceOfFunds === '' && this.updateForm.fundSourceList[0].value === '') { // 默认数据
            this.updateForm.fundSourceList = []
          } else if (this.updateForm.fundSourceList.length > 0) {
            let arr = []
            this.updateForm.fundSourceList.forEach(item => {
              if (item.sourceOfFunds !== '' && item.value !== '') {
                arr.push(item)
              }
            })
            this.updateForm.fundSourceList = arr
          }
        }
        // 路由地址的设定
