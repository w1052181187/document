        ...
        if (valid) {
          ...
          this.$confirm('确定要提交吗？', '提示', {
            ...
            if (this.flowType === 'tenderProject') {
              this.replyForm.noticeContent = '【项目管理】' + this.$store.getters.authUser.userName + '回复了您，去查看'
            } else if (this.flowType === 'workApproval') {
              this.replyForm.noticeContent = '【工作审批】' + this.$store.getters.authUser.userName + '回复了您，去查看'
            }
            if (Number(this.flowStatus) === 0 || Number(this.flowStatus) === 1) {
