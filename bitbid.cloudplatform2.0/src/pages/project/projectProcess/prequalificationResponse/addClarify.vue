<template>
  <div class="cloudcontent">
    <div class="main">
      ...
      <submitApproveDialog :tenderSystemCode="$route.query.tenderSystemCode" :showVisible="approveDialogVisable" @confirmSubmit="confirmSubmit" @handleCancel="cancelApprove"></submitApproveDialog>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getPrequalificationFileList () {
      ...
      documentInfo.getByRelaSection(query).then(res => {
        ...
        this.$set(this.updateForm, 'bidOpeningMethod', data[0].bidOpeningMethod)
        this.$set(this.updateForm, 'reviewMethod', data[0].reviewMethod)
        this.$set(this.updateForm, 'clarificationStartTime', data[0].clarificationStartTime)
        this.$set(this.updateForm, 'clarificationEndTime', data[0].clarificationEndTime)
        this.$set(this.updateForm, 'termOfValidity', data[0].termOfValidity)
        this.$set(this.updateForm, 'supplierCondition', data[0].supplierCondition)
        this.$set(this.updateForm, 'bidEvaluationMethod', data[0].bidEvaluationMethod)
        this.getBidSection()
      })
    }
  },
  mounted () {
    this.getPrequalificationFileList()
    this.getBidSections()
  }
}
</script>
