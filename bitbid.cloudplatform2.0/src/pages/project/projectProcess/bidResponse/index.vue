  ...
  <div class="cloudcontent" id="cloud_processtype">
    <div class="box" v-show="signUpVisable">
      <div class="topmain">
        投标受理信息
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="handleSignUpAdd" v-if="!tradingPlatformFlag">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 7) && tradingPlatformFlag" @click="handleAccept(scope)">受理</el-button>
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 7) && tradingPlatformFlag" @click="handleReject(scope)">驳回</el-button>
            <el-button type="text" size="small" @click="handleSignUpDetail(scope)">查看</el-button>
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 0 || scope.row.auditStatus === 3 || scope.row.auditStatus === 5)" @click="handleSignUpEdit(scope)">编辑</el-button>
            <el-button type="text" size="small" v-if="scope.row.auditStatus === 0" @click="handleDel(scope, 'signUp')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="scope.row.auditStatus === 1 && operationFlag && tradingPlatformFlag" @click="handleFileCostApprove(scope)">审批</el-button>
    ...
    <div class="box">
      <div class="topmain">
        招标文件疑义
        <!--按钮-->
        <el-button  type="primary" class="addbutton" @click="handleAddDoubt" v-if="operationFlag && !tradingPlatformFlag">
          <span> + 添加</span>
        </el-button>
        <!--按钮-->
      </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="(scope.row.auditStatus === 2 || scope.row.auditStatus === 4) && operationFlag" @click="handlePublishClarify(scope)">发布</el-button>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span  v-if="scope.row.paymentMethod === 6">保函</span>
            <span  v-if="scope.row.paymentMethod === 9">其他</span>
            <span  v-if="!scope.row.paymentMethod">---</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="auditStatus"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" v-if="scope.row.auditStatus === 1 && operationFlag && tradingPlatformFlag" @click="handleApprove(scope)">审批</el-button>
    ...
    <!--驳回弹框 -->
    <!--审批保证金缴纳记录弹框 -->
    <costRecordApproveDialog  :bidderName='curBondForm.bidderName' :amount="curBondForm.amount" :showVisible='dialogBondVisible' @handleConfirm="handleBondConfirm" @handleCancel="handleBondCancel"></costRecordApproveDialog>
    <!--审批保证金缴纳记录弹框 -->
    <!--审批招标文件发售记录弹框 -->
    <costRecordApproveDialog :bidderName='curCostApproveForm.bidderName' :amount="curCostApproveForm.amount" :showVisible='dialogFileCostVisible' @handleConfirm="handleFileCostConfirm" @handleCancel="handleFileCostCancel"></costRecordApproveDialog>
    <!--审批招标文件发售记录弹框 -->
  </div>
</template>
<script>
...
import costRecordApproveDialog from '@/pages/project/projectProcess/common/cost_record_approve_dialog.vue'
import {confirmSuccess} from '@/assets/js/utils'
export default {
  components: {
    applyAttachment,
    costRecordApproveDialog
  },
  ...
  methods: {
    ...
    handleApprove (scope) {
      this.curBondForm = scope.row
      this.dialogBondVisible = true
    },
    handleBondCancel () {
      this.dialogBondVisible = false
    },
    handleBondConfirm (obj) {
      let amount = this.curBondForm.amount
      let form = Object.assign(this.curBondForm, obj)
      if (Number(form.paymentStatus) === 1) {
        form.auditStatus = 2
        form.approverId = this.$store.getters.authUser.userId
        form.approver = this.$store.getters.authUser.userName
        form.approvalTime = new Date().getTime()
      } else {
        form.amount = amount
      }
      costInfo.gdApprove(form).then(res => {
        this.getBondInfoList()
        this.handleBondCancel()
      })
    },
    handleFileCostApprove (scope) {
      this.curCostApproveForm = scope.row
      this.dialogFileCostVisible = true
    },
    handleFileCostCancel () {
      this.dialogFileCostVisible = false
    },
    handleFileCostConfirm (obj) {
      let amount = this.curCostApproveForm.amount
      let form = Object.assign(this.curCostApproveForm, obj)
      if (Number(form.paymentStatus) === 1) {
        form.auditStatus = 2
        form.approverId = this.$store.getters.authUser.userId
        form.approver = this.$store.getters.authUser.userName
