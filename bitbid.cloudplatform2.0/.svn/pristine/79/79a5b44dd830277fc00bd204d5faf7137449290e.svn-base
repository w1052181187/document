<template>
  <div class="cloudcontent">
    ...
    <bidderCheckboxDialog :tenderSystemCode="tenderSystemCode" :showVisible="showBidderVisible" @selected="selectedBidder" @showDialog="showBidderDialog" v-if="checkDataSource && Number(updateForm.tenderMode) === 2"></bidderCheckboxDialog>
    <bidderRadioDialog :tenderSystemCode="tenderSystemCode" :showVisible="showBidderVisible" @selected="selectedBidder" @showDialog="showBidderDialog" v-if="checkDataSource && Number(updateForm.tenderMode) === 4"></bidderRadioDialog>
...
import bidderRadioDialog from '@/pages/project/projectProcess/common/bidder_radio_dialog.vue'
export default {
  components: {
    editor,
    uploadFile,
  ...
  methods: {
     ...
     */
    addSupplierBtn () {
      if (Number(this.updateForm.tenderMode) === 4 && this.updateForm.supplierBaseList.length === 1) {
        this.$message.warning('单一来源采购只允许添加一个供应商!')
        return false
      }
    ...
    submit (form, status) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (bidOpeningTime > biddingDocsEndTime && biddingDocsEndTime > biddingDocsBeginTime && biddingDocsBeginTime > currentDate) {
          } else {
            this.$message.warning('时间要求：开标时间 > 招标文件获取截止时间 > 招标文件获取开始时间 > 当前时间')
            return false
          }
          if (biddersInvitionEndTime > currentDate) {
          } else {
            this.$message.warning('时间要求：投标人邀请确认截止时间 > 当前时间')
            return false
          }
    ...
    // 选择投标人
    btnChoose (index) {
      if (index === -1 && Number(this.updateForm.tenderMode) === 4 && this.updateForm.supplierBaseList.length === 1) {
        this.$message.warning('单一来源采购只允许添加一个供应商!')
        return false
      }
    ...
    selectedBidder (obj) {
      if (!this.checkDataSource) { // 不推送
        ...
        /* let c = obj.concat(this.updateForm.supplierBaseList)
        let temp = c.map((item) => {
          return JSON.stringify(item)
        })
        temp = Array.from(new Set(temp))
        this.updateForm.supplierBaseList = temp.map((item) => {
          return JSON.parse(item)
        }) */
        if (Number(this.updateForm.tenderMode) === 2) {
          let arr = []
          for (let i = 0; i < obj.length; i++) {
            let flag = false
            for (let j = 0; j < this.updateForm.supplierBaseList.length; j++) {
              if (this.updateForm.supplierBaseList[j].supplierName === obj[i].name) {
                flag = true
                break
              }
            }
          ...
          this.updateForm.supplierBaseList = arr.concat(this.updateForm.supplierBaseList)
        } else {
          this.updateForm.supplierBaseList = []
          this.updateForm.supplierBaseList.push(obj)
        }
      }
    }
  },
