  ...
  <div class="tender-fee cloudcontent" id="tenderFee">
    <div class="main">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
          ...
          <el-col :span="12">
            <el-form-item label="缴费单位：" prop="bidderName">
              <el-input v-model="ruleForm.bidderName" readonly>
                <el-button type="primary" slot="append" size="mini" @click="companyChooseMtd">选择</el-button>
              </el-input>
        ...
        <el-form-item class="submit-radio">
          <el-button type="primary" v-if="Number(ruleForm.status) !== 1" @click="saveTenderFee('ruleForm', 0)">保存</el-button>
          <el-button type="primary" class="submitBtn" @click="saveTenderFee('ruleForm', 1)">提交</el-button>
    ...
    <el-dialog title="选择缴费单位" :visible.sync="companyChooseBox" class="company-choose-box" :width="'600px'">
      <div class="dialogUnaudited-content">
        <div>
          <el-input v-model="companyChooseName" style="width: 240px;"></el-input>
          <el-button type="primary" @click="companyChooseSrh">查询</el-button>
        ...
        <el-table v-if="isPaymentTable" :data="companyTableData" border tooltip-effect="dark" class=""
          ...
          <el-table-column fixed type="index" width="60" align="center" label="序号" :index='indexMethod'></el-table-column>
    ...
    <el-dialog title="选择缴费标段" :visible.sync="paymentTendersBox" class="company-choose-box" :width="'800px'">
      <div class="dialogUnaudited-content">
        <div class="payment-tender-box">
          <el-input v-model="paymentTendersName" placeholder="请输入标段名称" style="width: 240px;">
          </el-input>
          <el-button type="primary" @click="paymentTenderSrh">查询</el-button>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        bidSectionList: [{ required: true, trigger: ['change', 'blur'], validator: validArray }],
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$confirm(message, '提示', {
            ...
            if (!this.ruleForm.itemType) {
              this.ruleForm.itemType = 4
            }
    ...
    tableRowClick (row, event, column) {
      ...
      this.ruleForm.bidderId = row.objectId
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$confirm('确定要保存吗？', '提示', {
            ...
            this.bidderForm.enterpriseId = this.$store.getters.authUser.enterpriseId
            bidder.saveBidder(this.bidderForm).then((res) => {
              if (res.data.resCode === '0000') {
                this.ruleForm.bidderName = this.bidderForm.name
                this.ruleForm.bidderId = res.data.bidder.objectId
                this.addBidderBox = false
              }
            })
          })
